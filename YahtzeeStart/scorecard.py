...
def resetScorecard(scoreCard):
    for i in range (constants.ONES, constants.YAHTZEE):
        scoreCard[constants.USER].append(-1)

    for i in range(constants.SUBTOTAL, constants.TOTAL + 1):
        scoreCard[constants.USER].append(0)

    for i in range(constants.ONES, constants.YAHTZEE):
        scoreCard[constants.COMPUTER].append(-1)
    for i in range(constants.SUBTOTAL, constants.TOTAL + 1):
        scoreCard[constants.COMPUTER].append(0)
...
def updateScorecard(scoreCard):
    uScorecard = []
    cScorecard = []
    for i in range (constants.TOTAL):
        scoreCard[constants.USER].append(sum(uScorecard))
    for i in range (constants.TOTAL):
        scoreCard[constants.COMPUTER].append(sum(cScorecard))
